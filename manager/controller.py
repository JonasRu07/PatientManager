...
class Controller:
    def __init__(self) -> None:
        ...
        self.patient_manager = PatientWrapper(os.path.join(self.config_path, "patients.json"))
    ...
    def solve_define_answers(self) -> None:
        ...
        for hour in self.week.hours:
            patient_index = None
            patient_ref = None
            for index, patient in enumerate(self.patient_manager.patients):
                ...
                if patient_ref is not None:
                    ...
                    self.patient_manager.patients.remove(patient_ref)
                    changes = True

        return self.solve_define_answers() if changes else None
    
    def solve_recursive(self, pm:PatientWrapper, week:Week, start=0) -> list[Week,]:
        """
        Recursively generates all possible solutions to the calender and return them
        :param pm: PatientWrapper
        :param week: Week
        ...
        """
        if len(pm.patients) == 0:
            return [week]
        solutions = []
        for hour_index, hour in enumerate(week.hours):
            if hour_index < start or hour.taken_by is not None: continue
            for patient_index, patient in enumerate(pm.patients):
                if hour.ID in patient.pos_times:
                    new_week = week.copy()
                    new_week.hours[hour_index].taken_by = patient
                    new_pm = pm.copy()
                    new_pm.patients.pop(patient_index)
                    solutions += self.solve_recursive(new_pm, new_week, start=hour_index)
